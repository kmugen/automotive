...
Libraries/iLLD/TC27D/Tricore/Asclin/Lin/%.src: ../Libraries/iLLD/TC27D/Tricore/Asclin/Lin/%.c Libraries/iLLD/TC27D/Tricore/Asclin/Lin/subdir.mk
	...
	if [ -f "$(basename $@).d" ]; then sed.exe -r  -e 's/\b(.+\.o)\b/Libraries\/iLLD\/TC27D\/Tricore\/Asclin\/Lin\/\1/g' -e 's/\\/\//g' -e 's/\/\//\//g' -e 's/"//g' -e 's/([a-zA-Z]:\/)/\L\1/g' -e 's/\d32:/@TARGET_DELIMITER@/g; s/\\\d32/@ESCAPED_SPACE@/g; s/\d32/\\\d32/g; s/@ESCAPED_SPACE@/\\\d32/g; s/@TARGET_DELIMITER@/\d32:/g' "$(basename $@).d" > "$(basename $@).d_sed" && cp "$(basename $@).d_sed" "$(basename $@).d" && rm -f "$(basename $@).d_sed" 2>/dev/null; else echo 'No dependency file to process';fi
	@echo 'Finished building: $<'
	@echo ' '

Libraries/iLLD/TC27D/Tricore/Asclin/Lin/IfxAsclin_Lin.o: ./Libraries/iLLD/TC27D/Tricore/Asclin/Lin/IfxAsclin_Lin.src Libraries/iLLD/TC27D/Tricore/Asclin/Lin/subdir.mk
	@echo 'Building file: $<'
...
clean-Libraries-2f-iLLD-2f-TC27D-2f-Tricore-2f-Asclin-2f-Lin:
	-$(RM) ./Libraries/iLLD/TC27D/Tricore/Asclin/Lin/IfxAsclin_Lin.d ./Libraries/iLLD/TC27D/Tricore/Asclin/Lin/IfxAsclin_Lin.o ./Libraries/iLLD/TC27D/Tricore/Asclin/Lin/IfxAsclin_Lin.src
